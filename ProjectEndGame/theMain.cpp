...
// View matrix
		...
		//		if (collisionTest < 5.0f)
		//		{
		//			glm::vec3 iVelocityVector = glm::normalize(g_vec_pGameObjects.at(i)->getVelocity());

		//			glm::vec3 jVelocityVector = glm::normalize(g_vec_pGameObjects.at(j)->getVelocity());

		//			// closestTriangle.normal
		//			glm::vec3 reflectionVec = glm::reflect(iVelocityVector, jVelocityVector);
		...
		//			g_vec_pGameObjects.at(i)->setVelocity(reflectionVec * speed);
		//			g_vec_pGameObjects.at(j)->setVelocity(reflectionVec* speed);
		//		}
			}
		}
		...
		if (currentCommand == "followPath")
		{
			glm::vec3 currentPoint = gCoordinator->GetCurrentPathPoint();

			glm::mat4 pathPointModel = glm::mat4(1.0f);

			std::vector<glm::vec3> currentPath = gCoordinator->GetCurrentPath();
			...
			for (int i = 0; i < currentPath.size(); i++)
			{
				if (i != gCoordinator->GetCurrentPathSection())
				{
					...
					pathPoint->setPositionXYZ(currentPath.at(i));
					pathPoint->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
					pathPoint->setScale(10.0f);
			...
			pathPoint->setMeshName("sphere");
			pathPoint->setFriendlyName("debug_sphere");
			pathPoint->setPositionXYZ(currentPoint);
			pathPoint->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
			pathPoint->setScale(10.0f);
			//	pathPoint->objectColourRGBA = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f);
			...
			coordPoint->setPositionXYZ(gCoordinator->GetCoordinatorObject()->getPositionXYZ());
			coordPoint->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
			coordPoint->setScale(10.0f);
			//	pathPoint->objectColourRGBA = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f);
